...
Page({
  data: {
    ...
    sortType: '', // recommended,nearest,highest-rated
    foodList: [],
    isLoading: false,
    hasMore: true,
    page: 1,
  ...
  // 加载美食数据
  loadFoodData() {
    const encodedKeyword = encodeURIComponent(this.data.searchKeyword);
  ...
  changeSortType(e) {
    ...
    wx.request({
      url: `http://localhost:5299/api/merchants/types?type=${type}`,
      header: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + getApp().getToken(), // 携带 token
      },
      ...
      success: (res) => {
        console.log(res.data, 'meisjuti');
        if (res.data.statusCode == 200) {
          console.log(res.data, '这是今日推荐处查询')
          this.setData({
            sortType: type,
